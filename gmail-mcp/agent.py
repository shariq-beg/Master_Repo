...
- If the user gives dates, use search_gmail_date_window.
...
async def _classify_emails_with_qwen(
    ...
    llm = build_llm(model=model, temperature=0.0, max_tokens=768)
    ...
    for start in range(0, len(emails), MAX_CATEGORIZATION_BATCH):
        ...
        response = await llm.ainvoke(_classification_prompt(batch))
...
async def route_prompt(
    ...
    Used by:
        router_node.
    """
    llm = build_llm(model=model, temperature=0.0, max_tokens=64)
    ...
    try:
        ...
        return "chat_agent", "router returned invalid JSON"

    route = payload.get("route")
    if route not in {"chat_agent", "gmail_read_agent", "gmail_mutation_agent"}:
        return "chat_agent", "router returned unknown route"
